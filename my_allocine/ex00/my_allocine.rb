...
requests["Find the name of all reviewers who have rated 7 or more stars to their rating order by reviews rev_name (it might have duplicated names :-))"] = "SELECT rev_name FROM reviews INNER JOIN movies_ratings_reviews ON id = rev_id WHERE rev_stars >= '7' ORDER BY rev_name;"
...
requests["Find all the actors who have not acted in any movie between 1990 and 2000 (select only actor first name, last name, movie title and release year)"] = "SELECT act_fname, act_lname, mov_title,mov_year FROM movies,actors, movies_actors  WHERE movies_actors.mov_id=movies.id AND movies_actors.act_id=actors.id AND mov_year NOT BETWEEN 1990 AND 2000;"